...
tid_t process_fork(const char *name, struct intr_frame *if_ UNUSED)
{
	...
	struct thread *cur = thread_current();
	// 현재 스레드의 parent_if를 if_로 복사
	memcpy(&cur->parent_if, if_, sizeof(struct intr_frame));

	tid_t tid = thread_create(name, PRI_DEFAULT, __do_fork, cur); //__do_fork로 자식 스레드 생성
	if (tid == TID_ERROR)
	{
		return TID_ERROR;
	}
	struct thread *child = get_child_with_pid(tid);
	sema_down(&child->fork_sema); // 자식이 메모리에 로드될 때까지 대기
...
//부모의 page table을 복사해서 자신의 pml4를 만드는 함수
static bool duplicate_pte(uint64_t *pte, void *va, void *aux)
{
	...
	memcpy(newpage, parent_page, PGSIZE); //newpage에 부모 페이지를 복사
    writable = is_writable(pte); // pte는 parent_page를 가리키는 주소 
	/* 5. Add new page to child's page table at address VA with WRITABLE
	 *    permission. */
	if (!pml4_set_page(current->pml4, va, newpage, writable)) //pte가 writable 여부에 따라 자식의 writable 플래그 설정
	{
...
void argument_stack(char **argv, int argc, void **rsp)
{
	...
	if ((int)*rsp % 8)
	{
		...
		*rsp -= padding;
		memset(*rsp, 0, padding); // word-align
	}
	*rsp -= 8;
	memset(*rsp, 0, 8);
...
int process_wait(tid_t child_tid UNUSED)
{
	...
	if (child == NULL)
		return -1;
	sema_down(&child->wait_sema); // 자식이 자원을 획득한다. 자식이 자원을 획득할 때까지 부모는 대기
	// 부모 다시 깨어남 자식 프로세스는 이미 끝남
	int exit_status = child->exit_status; // child의 exit_status를 얻음
	list_remove(&child->child_elem);	  // 자식 리스트에서 해당 자식 삭제
	sema_up(&child->free_sema);			  // exit_status가 전달되었다고 자식에게 전달
	return exit_status;
}
...
void process_exit(void)
{
	...
	for (int i = 0; i < FDT_COUNT_LIMIT; i++)
	{
		close(i); // 바이크 단위로 하나하나 닫아준다 시스템콜
	}
	palloc_free_multiple(cur->fd_table, FDT_PAGES); // 페이지 해제로
	file_close(cur->running);						// 실행중에 수정 못하도록 파일 닫아주기
